...
- Removed the deterministic `review.py` driver and the `assess.mjs` scorer, and dropped the `pi-ai` dependency and `ARLANDRIA_MODEL*` vars (the LLM's model is Pi's).
...
- Self-bootstrapping primitives via `uv` + PEP 723 (no pip / venv); the `arlandria` launcher preflights for `uv`.
...
- Each review is now one self-contained folder (`<base>/<slug>/ledger.json` + `exports/` + `searches/` + `pdfs/`) at a user-chosen location (asked for at creation; default `$ARLANDRIA_HOME`, else `~/arlandria-reviews`). `export.py` writes into the review's `exports/` (with `--out-dir` / `--stdout`), so reviews no longer share a directory or overwrite each other's `references.*`.
...
## 0.1.0
- Initial scaffold: callimachus skill, ledger, multi-database search (PubMed, OpenAlex, Semantic Scholar, Europe PMC, arXiv).
