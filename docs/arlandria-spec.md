...
- `bin/callimachus.js` launches the **interactive** Pi REPL with the skill preloaded and the banner printed — and **stays** in the session. It does **not** run a single task and exit. The review is the `/callimachus` workflow invoked *within* the running session; between review turns the researcher can do any other Pi work and can interrogate/override the ledger at any point.
